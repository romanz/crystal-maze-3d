...
char *Fonts[] = { // GG
        ...
        "10x20",     /* Triplex font */
        "6x12",      /* Small font */
        "10x20",     /* Sansserif font */
        "12x24"      /* Gothic font */
};

struct bgi_info *vga_palette;
// GG
struct rgb_colour *rgb_palette;
		...
		case IBM8514:
	        ...
	        case X11:
				...
				case X11_USER: // GG
					...
					height = user_y;
			                break;
			        case X11_FULLSCREEN: // GG
			                width = 
			                  XDisplayWidth(dpy, screen_num);
					height = 
			                  XDisplayHeight(dpy, screen_num);
			                fullscreen = 1;
				default:
					break;
			}
...
// GG
        rgb_palette = (struct rgb_colour *) malloc((MAXRGBCOLORS + 1) * sizeof(struct rgb_colour));
	...
	// initialise the RGB palette - GG
	for (i = 0, RGBptr = rgb_palette; i <= MAXRGBCOLORS; i++, RGBptr++) {
                RGBptr->colour_index = i;
                RGBptr->pixel_value = -1;
        }
...
// GG
        if (fullscreen)
          XGrabKeyboard (dpy, window, True, // keyboard as usual
