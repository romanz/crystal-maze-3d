...
// Ported to Xbgi by Guido Gonzato, <guido.gonzato at gmail.com>
// Changes:
// 
// - commented out dos.h
// - 'far' undefined
// - 'USER_CHAR_SIZE' defined
// - itoa() added
// Fonts -> _Fonts
...
void ReportStatus(void)
{
  ...
  gprintf( &x, &y, "Current font       : %s", _Fonts[ textinfo.font ] );
...
void TextDump()
{
  ...
  for( font=0 ; font<5 ; ++font ){	/* For each available font	*/
    sprintf( buffer, "%s Character Set", _Fonts[font] );
...
void TextDemo(void)
{
  ...
  for( font=0 ; font<5 ; ++font ){	/* For each of the four fonts	*/
    ...
    sprintf( buffer, "%s Demonstration", _Fonts[font] );
    MainWindow( buffer );
    getviewsettings( &vp );
...
void PaletteDemo(void)
{
  ...
  while( !kbhit() ){			/* Until user enters a key...	*/
    ...
    setpalette( 1+random(MaxColors - 2), 1+random(MaxColors - 2) );
  }
...
int gprintf( int *xloc, int *yloc, char *fmt, ... )
{
  ...
  // FIXED: was va_start( argptr, format );
